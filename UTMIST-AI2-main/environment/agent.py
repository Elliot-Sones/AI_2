...
class RewardManager():
    ...
    def process(self, env, dt) -> float:
        ...
        log = env.logger[0]
        # Fix: Handle torch tensors properly
        if isinstance(reward_buffer, torch.Tensor):
            reward_buffer_val = reward_buffer.item() if reward_buffer.numel() == 1 else reward_buffer[0].item()
        else:
            reward_buffer_val = float(reward_buffer)
        
        if isinstance(self.total_reward, torch.Tensor):
            total_reward_val = self.total_reward.item() if self.total_reward.numel() == 1 else self.total_reward[0].item()
        else:
            total_reward_val = float(self.total_reward)
        
        log['reward'] = f'{reward_buffer_val:.3f}'
        log['total_reward'] = f'{total_reward_val:.3f}'
        env.logger[0] = log
        
        # Return scalar value
        if isinstance(reward, torch.Tensor):
            return reward.item() if reward.numel() == 1 else reward[0].item()
        return float(reward)
