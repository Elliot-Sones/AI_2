...
# Helper function to resolve asset paths relative to this file
...
class UIHandler():
    ...
    def __init__(self, camera):
        ...
        SCALE_FACTOR = 0.11
        self.agent_1_score = pygame.image.load(_get_asset_path('environment/assets/ui/player1ui.png'))
        ...
        self.agent_2_score = pygame.image.load(_get_asset_path('environment/assets/ui/player2ui.png'))
        ...
        self.life = pygame.image.load(_get_asset_path('environment/assets/ui/alicon_alive.png'))
        ...
        self.death = pygame.image.load(_get_asset_path('environment/assets/ui/alicon_dead.png'))
...
class Camera():    
    ...
    def scale_background(self, env, bg_image: pygame.image = pygame.image.load(_get_asset_path('environment/assets/map/background.png'))) -> None:
...
class WarehouseBrawl(MalachiteEnv[np.ndarray, np.ndarray, int]):
    ...
    def load_attacks(self):
        ...
        for file in sorted(os.listdir(_get_asset_path('environment/unarmed_attacks'))):
            name = file.split('.')[0]
            ...
            move_data = None
            try:
                with open(os.path.join(_get_asset_path('environment/unarmed_attacks'), file)) as f:
                    move_data = json.load(f)
            except Exception as e:
                print(f"Error loading {file}: {e}")
            self.attacks[self.keys[name]] = move_data 

        for file in sorted(os.listdir(_get_asset_path('environment/spear_attacks'))):
            name = file.split('.')[0].split(" ")[1]
            if name not in self.keys.keys(): continue
            move_data = None
            try:
                with open(os.path.join(_get_asset_path('environment/spear_attacks'), file)) as f:
                    move_data = json.load(f)
            except Exception as e:
                print(f"Error loading {file}: {e}")
        ...
        for file in sorted(os.listdir(_get_asset_path('environment/hammer_attacks'))):
            name = file.split('.')[0].split(" ")[1]
            if name not in self.keys.keys(): continue
            move_data = None
            try:
                with open(os.path.join(_get_asset_path('environment/hammer_attacks'), file)) as f:
                    move_data = json.load(f)
            except Exception as e:
                print(f"Error loading {file}: {e}")
...
class Ground(GameObject):
    ...
    def load_assets(self):
        if self.loaded:
            return
        self.loaded = True
        self.bg_img = pygame.image.load(_get_asset_path('environment/assets/map/bg.jpg'))
        self.stage_img = pygame.image.load(_get_asset_path('environment/assets/map/stage.png'))
...
class Stage(GameObject):
    ...
    def load_assets(self):
        ...
        self.loaded = True
        self.bg_img = pygame.image.load(_get_asset_path('environment/assets/map/bg.jpg'))
        self.platform_img = pygame.image.load(_get_asset_path('environment/assets/map/platform.png'))
...
class Player(GameObject):
    ...
    def __init__(self, env, agent_id: int, start_position=[0,0], color=[200, 200, 0, 255]):
        ...
        if not os.path.exists(animation_folder):
            self.load_assets()
        self.animation_sprite_2d = AnimationSprite2D(self.env.camera, 1.0, _get_asset_path('environment/assets/player'), agent_id)
        self.attack_sprite = AnimationSprite2D(self.env.camera, 2.0, _get_asset_path('environment/assets/attacks'), agent_id)
       ...
       #  self.draw_image(camera.canvas, self.frames[self.current_frame_index], self.position, self.scale * width, camera, flipped=flipped)
        if not isinstance(self.state, AttackState) and not issubclass(self.state.__class__, AttackState):
            if(self.weapon in ["Hammer","Spear"]):
                image = pygame.image.load(_get_asset_path(f"environment/assets/weapons/{self.weapon}.png"))
